...
class SecretCtl:
    ...
    def add_key(self, key_type, name, key, identity_path=None):
        ...
        if key_type not in ["user", "host"]:
            print(
                f"错误: 无效的密钥类型 '{key_type}'，支持: user, host", file=sys.stderr
            )
            return False
    ...
    def remove_key(self, key_type, name):
        ...
        try:
            # 确认
            response = input(
                f"确定要删除{key_type}密钥 '{name}'? 这可能会导致加密文件无法解密。(y/N): "
            )
            if response.lower() != "y":
                print("已取消")
                return False
    ...
    def add_secret_mapping(
        self, secret_name, recipients, target_path, owner, group, mode="600"
    ):
        """添加新的密钥映射"""
    ...
    "{secret_name}" = {{
      ...
      mode = "{mode}";
        ...
        def update_content(content):
            ...
            return re.sub(
                mappings_pattern, f"\\1{mapping_entry}\n  \\2", content, flags=re.DOTALL
            )
...
def main():
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="模拟执行，不实际更改文件"
    )
    ...
    list_parser.add_argument(
        "type", choices=["keys", "secrets", "all"], help="列出类型"
    )
    list_parser.add_argument(
        "--filter",
        choices=["all", "users", "hosts", "groups"],
        default="all",
        help="密钥过滤器",
    )
    ...
    encrypt_parser.add_argument(
        "--recipients", nargs="+", required=True, help="接收者公钥"
    )
    ...
    rekey_parser.add_argument(
        "--recipients", nargs="+", required=True, help="新接收者公钥"
    )
    ...
    if args.command == "list":
        ...
        secret_ctl.add_secret_mapping(
            args.name, args.recipients, args.target, args.owner, args.group, args.mode
        )
