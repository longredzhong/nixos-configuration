...
| [Memoh](memoh.md) | 多智能体平台：为什么放在这台客户机、声明式构成、上游与环境的四个坑、暴露面与验证 | `hosts/longred-vm/memoh.nix`、`secrets/memoh-env.age` |
| [Agent 记忆服务（方案）](agent-memory.md) | 跨 harness、跨项目共用长期记忆的目标架构、社区候选评估与分阶段落地路径；当前为方案，未实施 | 方案文档；复用 `modules/host-services/` 与 `hosts/longred-vm/` 的既有模式 |
