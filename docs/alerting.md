...
两者都表现为"服务端一切正常、应用里能看到消息、**锁屏没有任何通知**"，排查时容易全部怀疑服务端：

1. **App 里填的服务器地址必须与 `base-url` 逐字一致。** iOS 端把「服务器地址 + 主题」做 SHA256 得到 Firebase 主题，而它只做一件事：去掉尾部斜杠（`normalizeBaseUrl`），**不补协议、不做其它规范化**。所以填 `http://…` 或省略 `https://` 时哈希就对不上——但**轮询照样能用**（访问会被 301 到 https），于是应用内看得到、推送收不到。改完地址后**必须删掉订阅重新添加**：Firebase 主题只在添加订阅时注册一次，改地址或改主题都不会自动重算。
2. **通知权限被拒时 iOS 直接丢弃远程通知**，不报错、不提示。检查 iOS 设置 → 通知 → 对应客户端，同时确认"定时推送摘要"没有把它收进去、没有专注模式在拦。

判别方法：把应用切到**前台**再发一条消息。前台能弹出横幅说明主题匹配、链路通，问题在后台展示；前台也不弹、只有列表更新，则更可能是地址字符串导致的主题不匹配。
